...
describe('ConfigurationForm component', () => {
  ...
  test('addMetrics enabled flow', async () => {
    // given
    const user = userEvent.setup();
    render(<ConfigurationForm />);
    ...
    // then
    expect(
      within(screen.getByRole('radiogroup', { name: /Add metrics endpoints/i })).getByRole('radio', { name: /yes/i })
    ).not.toBeDisabled();
    expect(
      within(screen.getByRole('radiogroup', { name: /Add metrics endpoints/i })).getByRole('radio', { name: /no/i })
    ).not.toBeDisabled();
  });

  test('addMetrics disabled flow', async () => {
    // given
    const user = userEvent.setup();
    render(<ConfigurationForm />);
